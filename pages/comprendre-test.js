...
const Pourquoi = ({ translation }) => (
  <div style={{ textAlign: "justify" }}>
    <p>
      ...
      <span className="font-weight-bold">EPDS</span>
    </p>
    <p>
      <span className="font-weight-bold">
        {translation("introduction.explication.partie1")}{" "}
      </span>
    ...
    <p>
      ...
      <span className="font-weight-bold">
        {translation("introduction.diagnostic.partie2")}
      </span>
      {translation("introduction.diagnostic.partie3")}
      <span className="font-weight-bold">
        {translation("introduction.diagnostic.partie4")}
      </span>
    </p>
  </div>
)
...
function FormInformations(props) {
  ...
  return (
    <div>
      <div className="page-subtitle comprendre-test-informations">
        Informations
      </div>
      <form onSubmit={props.onclick}>
        <div
          className={`form-group fr-input-group ${
            isGenderValid ? "fr-input-group--valid" : ""
          }`}
        >
          <label className="fr-label" htmlFor="genderSelect text-input-valid">
            {props.translation("genre")}
          </label>
          <select
            className="form-control custom-select col-sm-5"
            id="genderSelect"
            name="genderSelect"
            onBlur={handleChange}
            required
          >
            <option value={EpdsGender.inconnu.strapiLibelle}>
              {props.translation("hint-genre")}
            </option>
            <option value={EpdsGender.feminin.strapiLibelle}>
              {props.translation("common:" + EpdsGender.feminin.key)}
            </option>
            <option value={EpdsGender.masculin.strapiLibelle}>
              {props.translation("common:" + EpdsGender.masculin.key)}
            </option>
            <option value={EpdsGender.nonBinaire.strapiLibelle}>
              {props.translation("common:" + EpdsGender.nonBinaire.key)}
            </option>
          </select>
        </div>
        <Row>
          <Col
            className={`form-group fr-input-group input-name ${
              isLastnameValid ? "fr-input-group--valid" : ""
            }`}
          >
            <label className="fr-label" htmlFor="text-input-valid">
              {props.translation("prenom")}
            </label>
            <input
              type="text"
              className={`form-control fr-input custom-input ${
                isLastnameValid ? "custom-input-valid" : ""
              }`}
              id="inputLastname"
              name="inputLastname"
          ...
          <Col
            className={`form-group fr-input-group input-name ${
              isNameValid ? "fr-input-group--valid" : ""
            }`}
          >
            <label className="fr-label" htmlFor="text-input-valid">
              {props.translation("nom")}
            </label>
            <input
              type="text"
              className={`form-control fr-input custom-input ${
                isNameValid ? "custom-input-valid" : ""
              }`}
              id="inputName"
              name="inputName"
              onChange={handleChange}
        ...
        <div className="champs-obligatoires">
          {props.translation("champs-obligatoire")}
        </div>
        <button
          type="submit"
          className="fr-btn"
